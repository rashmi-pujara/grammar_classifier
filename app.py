...
model_name_grammar_check = 'random_forest_grammar_check.joblib'
rf_grammar_check = joblib.load(model_name_grammar_check)
...
def correct_grammar_suggestion(num_return_sequences=2, model=model):
    try:
        ...
        if(qa["output"] == 1 and input[-1]!='?'):
            temp_input += "?"
        cg = correct_grammar_or_not(temp_input)
        if(cg["output"] == 0):
            ...
            return {"opration": "success", "grammar_corrected" : 1,"input": input, "output": tgt_text}
        else:
            return {"opration": "success", "grammar_corrected" : 0,"input": input, "output": input}
...
@app.route("/api/correct_grammar_or_not", methods=["POST"], defaults={'sentence': None})
def correct_grammar_or_not(sentence):
    try:
        if(sentence == None):
            sentence = request.form["sentence"]
        emb = embedder.encode(sentence)
        output = rf_grammar_check.predict([emb])[0]
        return {"opration": "success", "input": sentence, "output": str(output)}
    except Exception as e:
        return {"opreation": "failed", "error": traceback.format_exc()}
